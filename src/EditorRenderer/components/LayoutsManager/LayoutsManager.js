...
class LayoutsManager extends Component {
    ...
    loadPageHandler = () => {
        axios.get('https://localhost:5001/api/layouts/' + encodeURIComponent(this.state.selectedPage),{
            headers: {
            'Authorization': `Bearer ${this.props.token}` 
          }}).then(response => {
    ...
    removePageHandler = () => {
        axios.delete('https://localhost:5001/api/layouts/delete/' + encodeURIComponent(this.state.selectedPage),{
            headers: {
            'Authorization': `Bearer ${this.props.token}` 
          }});
    ...
    savePageHandler = () => {
        axios.put('https://localhost:5001/api/layouts/put', {name:this.state.currentPage, content: JSON.stringify(this.props.design) },{
            headers: {
            'Authorization': `Bearer ${this.props.token}` 
          }}
        ).then(response1 => {
            axios.get("https://localhost:5001/api/layouts").then(response2 => {
