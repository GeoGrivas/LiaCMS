...
class LayoutsManager extends Component {
    ...
    loadPageHandler = () => {
        axios.get('https://www.adventurouscoding.com/api/layouts/' + encodeURIComponent(this.state.selectedPage),{
            headers: {
            'Authorization': `Bearer ${this.props.token}` 
          }}).then(response => {
    ...
    removePageHandler = () => {
        axios.delete('https://www.adventurouscoding.com/api/layouts/delete/' + encodeURIComponent(this.state.selectedPage),{
            headers: {
            'Authorization': `Bearer ${this.props.token}` 
          }});
    ...
    savePageHandler = () => {
        axios.put('https://www.adventurouscoding.com/api/layouts/put', {name:this.state.currentPage, content: JSON.stringify(this.props.design) },{
            headers: {
            'Authorization': `Bearer ${this.props.token}` 
          }}
        ).then(response1 => {
            axios.get("https://www.adventurouscoding.com/api/layouts").then(response2 => {
